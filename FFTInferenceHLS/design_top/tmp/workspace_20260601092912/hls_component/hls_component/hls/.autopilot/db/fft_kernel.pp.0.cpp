...
# 2 "<built-in>" 2
# 1 "../../../../src/fft_kernel.cpp" 2
# 1 "../../../../src/fft_kernel.h" 1
...
# 10 "/opt/Xilinx/Vitis_HLS/2024.1/common/technology/autopilot/ap_fixed.h" 2
...
# 11 "/opt/Xilinx/Vitis_HLS/2024.1/common/technology/autopilot/ap_int.h"
...
# 5 "../../../../src/fft_kernel.h" 2
# 1 "/opt/Xilinx/Vitis_HLS/2024.1/common/technology/autopilot/ap_int.h" 1
# 6 "../../../../src/fft_kernel.h" 2
...
extern "C" {
__attribute__((sdx_kernel("fft_kernel", 0))) void fft_kernel(
    fixed_t *input_real,
    fixed_t *input_imag,
    fixed_t *output_real,
    fixed_t *output_imag
...
# 2 "../../../../src/fft_kernel.cpp" 2
# 1 "../../../../src/fft_luts.h" 1



static const fixed_t hamming[1024] = {
  1311,
  1311,
  1311,
  1312,
...
# 3 "../../../../src/fft_kernel.cpp" 2



static int bit_reverse(int x) {
    int y = 0;
    VITIS_LOOP_8_1: for (int i = 0; i < 10; i++) {
        y = (y << 1) | (x & 1);
        x >>= 1;
    }
    return y;
...
#line 1 "directive"
#pragma HLSDIRECTIVE TOP name=fft_kernel
# 21 "../../../../src/fft_kernel.cpp"
...
#pragma HLS INTERFACE s_axilite port=return bundle=control
 ...
 fixed_t real[1024];
    fixed_t imag[1024];
    fixed_t tmp_real[1024];
    fixed_t tmp_imag[1024];
    fixed_t out_real[1024];
    fixed_t out_imag[1024];
...
#pragma HLS BIND_STORAGE variable=out_imag type=ram_2p impl=bram
 ...
 VITIS_LOOP_47_1: for (int f = 0; f < 50; f++) {

        VITIS_LOOP_49_2: for (int i = 0; i < 1024; i++) {
#pragma HLS PIPELINE II=1
 real[i] = input_real[f * 1024 + i];
            imag[i] = input_imag[f * 1024 + i];
        }
...
#pragma HLS PIPELINE II=1
 fixed_t w = hamming[i];
        ...
        VITIS_LOOP_62_4: for (int i = 0; i < 1024; i++) {
#pragma HLS PIPELINE II=1
 int rev = bit_reverse(i);
        ...
        VITIS_LOOP_69_5: for (int i = 0; i < 1024; i++) {
#pragma HLS PIPELINE II=1
 real[i] = tmp_real[i];
            imag[i] = tmp_imag[i];
        }

        VITIS_LOOP_75_6: for (int s = 0; s < 10; s++) {
            int half_stride = 1 << s;
            int stride = 1 << (s + 1);
            int tw_step = 1 << (10 - 1 - s);

            VITIS_LOOP_80_7: for (int k = 0; k < 512; k++) {
#pragma HLS PIPELINE II=1
 int group = k >> s;
                ...
                fixed_t bw_real =
                    ((b_real * w_real) - (b_imag * w_imag)) >> 14;

                fixed_t bw_imag =
                    ((b_real * w_imag) + (b_imag * w_real)) >> 14;
            ...
            VITIS_LOOP_109_8: for (int i = 0; i < 1024; i++) {
#pragma HLS PIPELINE II=1
 real[i] = out_real[i];
                imag[i] = out_imag[i];
            }
        ...
        VITIS_LOOP_116_9: for (int i = 0; i < 1024; i++) {
#pragma HLS PIPELINE II=1
 output_real[f * 1024 + i] = real[i];
            output_imag[f * 1024 + i] = imag[i];
        }
    }
}
}
